...
namespace NoLock.Social.Core.Tests.Accessibility.Services
{
    public class AnnouncementServiceTests : IDisposable
    {
        ...
        [Fact]
        public async Task ClearAnnouncementsAsync_WithExceptionInHandler_LogsErrorAndContinues()
        {
            // Arrange
            _service.OnPoliteAnnouncement += (sender, args) => throw new InvalidOperationException("Handler error");
            ...
            _mockLogger.Verify(
                x => x.Log(
                    LogLevel.Error,
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("Error clearing announcements")),
                    It.IsAny<Exception>(),
                    It.IsAny<Func<It.IsAnyType, Exception?, string>>()),
                Times.Once);
        }
        ...
        [InlineData(AnnouncementCategory.CameraOperation)]
        [InlineData(AnnouncementCategory.QualityFeedback)]
        public async Task AnnouncementCategories_AllEnumValuesSupported(AnnouncementCategory category)
        {
            // Arrange
            const string message = "Category test message";
        ...
        [Fact]
        public async Task TimestampAccuracy_PoliteAndAssertiveAnnouncements_WithinReasonableRange()
        {
            // Arrange
            var beforeTime = DateTime.UtcNow;
            const string politeMessage = "Polite timestamp test";
            const string assertiveMessage = "Assertive timestamp test";

            // Act
            await _service.AnnouncePoliteAsync(politeMessage);
            await _service.AnnounceAssertiveAsync(assertiveMessage);
            ...
            var politeTimestamp = _capturedPoliteAnnouncements[0].Timestamp;
            var assertiveTimestamp = _capturedAssertiveAnnouncements[0].Timestamp;
            
            politeTimestamp.Should().BeOnOrAfter(beforeTime);
            politeTimestamp.Should().BeOnOrBefore(afterTime);
            assertiveTimestamp.Should().BeOnOrAfter(beforeTime);
            assertiveTimestamp.Should().BeOnOrBefore(afterTime);
        }

        [Fact]
        public async Task ConcurrentAnnouncements_ThreadSafety_HandlesMultipleSimultaneousRequests()
        {
            // Arrange
            ...
            {
                var index = i;
                tasks.Add(_service.AnnouncePoliteAsync($"Concurrent message {index}"));
                tasks.Add(_service.AnnounceAssertiveAsync($"Concurrent assertive {index}"));
            }

            await Task.WhenAll(tasks);

            // Assert
            _capturedPoliteAnnouncements.Should().HaveCount(concurrentRequests * 2); // 2 events per announcement (announce + clear)
            _capturedAssertiveAnnouncements.Should().HaveCount(concurrentRequests * 2);
            
            // Verify all messages were captured
            var politeMessages = _capturedPoliteAnnouncements.Where(a => !string.IsNullOrEmpty(a.Message)).Select(a => a.Message);
            var assertiveMessages = _capturedAssertiveAnnouncements.Where(a => !string.IsNullOrEmpty(a.Message)).Select(a => a.Message);
            
            politeMessages.Should().HaveCount(concurrentRequests);
            assertiveMessages.Should().HaveCount(concurrentRequests);
        }

        [Fact]
        public async Task MemoryEfficiency_LargeNumberOfAnnouncements_DoesNotCauseMemoryLeaks()
        {
            // Arrange
            const int largeNumberOfAnnouncements = 1000;
            var tasks = new List<Task>();

            // Act
            for (int i = 0; i < largeNumberOfAnnouncements; i++)
            {
                tasks.Add(_service.AnnouncePoliteAsync($"Message {i}"));
            }
            ...
            _capturedPoliteAnnouncements.Count.Should().BeGreaterThanOrEqualTo(largeNumberOfAnnouncements); // At least one event per message
            _capturedPoliteAnnouncements.Count.Should().BeLessThanOrEqualTo(largeNumberOfAnnouncements * 2); // At most two events per message
        ...
        [InlineData("\n")]
        [InlineData("\r\n")]
        public async Task WhitespaceOnlyMessages_TreatedAsEmpty(string whitespaceMessage)
        {
            // Act
            await _service.AnnouncePoliteAsync(whitespaceMessage);

            // Assert - Should be treated as empty and not announce
            _capturedPoliteAnnouncements.Should().BeEmpty();
            _mockLogger.Verify(
                x => x.Log(
                    LogLevel.Warning,
